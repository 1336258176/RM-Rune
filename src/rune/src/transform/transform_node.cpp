...
void TransformNode::fanbladeCallback(const rune_sys_interfaces::msg::Fanblade::ConstSharedPtr & msg)
{
  ...
  if (!std::isnormal(msg->r_center.position.x) || !std::isnormal(msg->r_center.position.y) ||
      !std::isnormal(msg->r_center.position.z))
  {
    RCLCPP_WARN(this->get_logger(), "fanblade-msg r_center isnt normal.");
    return;
  }
  if (!std::isnormal(msg->fanblade_center.position.x) ||
      !std::isnormal(msg->fanblade_center.position.y) ||
      !std::isnormal(msg->fanblade_center.position.z))
  {
    RCLCPP_WARN(this->get_logger(), "fanblade-msg fanblade_center isnt normal.");
    return;
  }
  ...
  const double r_center[3] = {
    msg->r_center.position.x, msg->r_center.position.y, msg->r_center.position.z};
  double r_center_odom[3];
  frameTransform2Odom(r_center, r_center_odom, "camera_frame", msg->header.stamp);

  const double fanblade_center[3] = {msg->fanblade_center.position.x,
                                     msg->fanblade_center.position.y,
                                     msg->fanblade_center.position.z};
  double fanblade_center_odom[3];
  frameTransform2Odom(fanblade_center, fanblade_center_odom, "camera_frame", msg->header.stamp);

  rune_sys_interfaces::msg::Fanblade fanblade_msg;
  fanblade_msg.header.stamp               = msg->header.stamp;
  fanblade_msg.header.frame_id            = "odom_frame";
  fanblade_msg.r_center.position.x        = r_center_odom[0];
  fanblade_msg.r_center.position.y        = r_center_odom[1];
  fanblade_msg.r_center.position.z        = r_center_odom[2];
  fanblade_msg.fanblade_center.position.x = fanblade_center_odom[0];
  fanblade_msg.fanblade_center.position.y = fanblade_center_odom[1];
